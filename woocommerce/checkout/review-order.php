...
<div class="shop_table woocommerce-checkout-review-order-table">
	...
	<h2 class="text-white text-xl md:text-2xl font-bold mb-4">Ваш заказ</h2>
	<div class="h-[2px] w-full bg-[#FF6B00]"></div>
	...
	<div class="flex flex-col">
		...
		foreach ( $visible_items as $index => $item ) {
			...
			?>
			<div class="py-5 <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
				<div class="flex items-start justify-between gap-4">
					<div class="min-w-0">
						<p class="text-white text-base font-bold leading-snug"><?php echo esc_html( $_product->get_name() ); ?></p>
			...
			<?php if ( ! $is_last ) : ?>
				<div class="h-[2px] w-full bg-[#FF6B00]" aria-hidden="true"></div>
			<?php endif; ?>
			<?php
		}
		?>
	...
	<div class="flex items-baseline justify-between gap-4 py-6 order-total-row">
		<span class="text-[#B8C0CC] text-lg md:text-xl font-bold">Итого:</span>
