...
<div class="w-full px-[10px] md:px-0 pt-[110px] md:pt-[130px] pb-10 md:pb-14">
	<div class="max-w-[1200px] mx-auto">
		<nav class="flex flex-wrap items-center gap-2 text-sm mb-6 md:mt-[90px] md:mb-8" aria-label="<?php esc_attr_e( 'Breadcrumb', 'motorcycle-shop' ); ?>">
				...
				<div class="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 lg:gap-10 items-start">
					<div class="flex flex-col gap-4" data-cart-items>
						...
						foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
							...
							?>
							<article class="woocommerce-cart-form__cart-item bg-[#2A3038] border border-[#434C58]/50 rounded-[2px] p-4 md:p-5 flex flex-col sm:flex-row gap-4 sm:gap-5 <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
								<div class="flex gap-4 flex-1 min-w-0">
									<?php if ( $product_permalink ) : ?>
										<a href="<?php echo esc_url( $product_permalink ); ?>" class="shrink-0 w-[100px] h-[80px] md:w-[120px] md:h-[90px] rounded-[2px] overflow-hidden bg-[#1F242B]">
											<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $product_name ); ?>" class="w-full h-full object-cover" />
										</a>
									<?php else : ?>
										<div class="shrink-0 w-[100px] h-[80px] md:w-[120px] md:h-[90px] rounded-[2px] overflow-hidden bg-[#1F242B]">
											<img src="<?php echo esc_url( $image_url ); ?>" alt="<?php echo esc_attr( $product_name ); ?>" class="w-full h-full object-cover" />
										</div>
									<?php endif; ?>
									...
									<div class="flex flex-col min-w-0 flex-1">
										<?php if ( $product_permalink ) : ?>
											<a href="<?php echo esc_url( $product_permalink ); ?>" class="text-white text-lg md:text-xl font-bold hover:text-[#FB8A3C] transition-colors leading-snug">
												<?php echo esc_html( $product_name ); ?>
											</a>
										<?php else : ?>
											<h2 class="text-white text-lg md:text-xl font-bold leading-snug"><?php echo esc_html( $product_name ); ?></h2>
										<?php endif; ?>

										<?php if ( $subtitle ) : ?>
											<p class="text-[#B8C0CC] mb-4 text-sm mt-1 leading-relaxed"><?php echo esc_html( $subtitle ); ?></p>
										<?php endif; ?>

										<?php echo wc_get_formatted_cart_item_data( $cart_item ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>

										<div class="flex items-center gap-4 mt-4 sm:mt-auto flex-wrap">
											...
											<div class="text-white text-xl md:text-2xl font-bold sm:ml-auto">
								...
								<button
									...
									class="self-start sm:self-center shrink-0 p-2 text-white/70 hover:text-[#FF6B00] transition-colors"
