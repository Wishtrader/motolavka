...
<div class="w-full fluid-px lg:pt-[110px] md:pt-[130px] pb-10 md:pb-14">
	<div class="max-w-[1200px] mx-auto">
		<nav class="flex items-center gap-2 text-[13px] mb-[60px] mt-[80px]" aria-label="<?php esc_attr_e(
    		'Breadcrumb',
    		'motorcycle-shop',
		); ?>">
			<a href="<?php echo
		...
		<h1 class="text-white text-[34px] md:text-[52px] font-bold leading-tight mb-8 md:mb-10">Корзина</h1>
		...
		<div data-cart-page-root class="transition-opacity">
			<form class="woocommerce-cart-form motorcycle-shop-cart-form" action="<?php echo
    			esc_url(wc_get_cart_url())
			; ?>" method="post">
				<?php do_action('woocommerce_before_cart_table'); ?>

				<div class="grid grid-cols-1 lg:grid-cols-[1fr_384px] gap-8 lg:gap-5 items-start">
					<div class="flex flex-col gap-4" data-cart-items>
						...
						foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
    						...
    						?>
							<article class="woocommerce-cart-form__cart-item bg-[#2A3038] border border-[#434C58]/50 rounded-[2px] p-4 <?php echo
								...
								<div class="flex flex-col lg:flex-row gap-4 lg:gap-10 items-start lg:items-center">
									...
									<div class="w-full lg:w-auto shrink-0">
										<?php if ($product_permalink): ?>
											...
											; ?>" class="block w-full h-[140px] md:h-[160px] lg:w-[160px] lg:h-[120px] rounded-[8px] overflow-hidden bg-[#1F242B] bg-cover">
												<img src="<?php echo esc_url($image_url); ?>" alt="<?php echo
    												esc_attr($product_name)
												; ?>" class="w-full rounded-[8px] h-full object-cover" />
											</a>
										<?php else: ?>
									...
									<div class="flex-1 min-w-0 lg:max-w-[200px]">
										<?php if ($product_permalink): ?>
											<a href="<?php echo
    											esc_url($product_permalink)
											; ?>" class="block text-white text-[20px] md:text-lg lg:text-xl font-normal hover:text-[#FB8A3C] transition-colors truncate">
												<?php echo esc_html($product_name); ?>
											</a>
										<?php else: ?>
											<h2 class="text-white text-[20px] md:text-lg lg:text-xl font-normal truncate"><?php echo
...
<div class="flex items-center gap-4 justify-between w-full lg:w-auto lg:gap-6 lg:shrink-0">
									...
									<div class="shrink-0">
										<div class="flex items-stretch h-[44px]">
											<button type="button" data-cart-qty-minus class="w-11 bg-[#1F242B] text-white text-[24px] hover:bg-[#111317] transition-colors rounded-l-[2px]" aria-label="<?php esc_attr_e(
    											'Уменьшить',
    											'motorcycle-shop',
											); ?>">−</button>
											<label class="sr-only" for="cart-qty-<?php echo esc_attr($cart_item_key); ?>"><?php esc_html_e(
    											'Количество',
    											'motorcycle-shop',
											); ?></label>
											<input
												...
												inputmode="numeric"
											/>
											<button type="button" data-cart-qty-plus class="w-11 bg-[#1F242B] text-white text-[24px] hover:bg-[#111317] transition-colors rounded-r-[2px]" aria-label="<?php esc_attr_e(
    											'Увеличить',
    											'motorcycle-shop',
											); ?>">+</button>
										</div>
