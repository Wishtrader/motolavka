...
<div class="cart_totals bg-[#2A3038] border border-[#434C58]/50 rounded-[2px] p-2.5 md:p-6 <?php echo
    WC()->customer->has_calculated_shipping() ? 'calculated_shipping' : ''
; ?>">
	...
	<?php foreach (WC()->cart->get_coupons() as $code => $coupon): ?>
		<div class="flex items-center justify-between gap-4 text-sm mb-2 text-[#B8C0CC] coupon-<?php echo
    		esc_attr(sanitize_title($code))
		; ?>">
	...
	<div class="wc-proceed-to-checkout flex flex-col gap-3 mb-6">
		...
		motorcycle_shop_lead_modal_trigger(array(
    		'source' => 'cart',
    		'class' => 'flex w-full min-h-[52px] items-center justify-center rounded-[2px] bg-transparent text-white text-sm font-semibold border border-[#434C58] hover:bg-[#1F242B] transition-colors',
		));
		?>
	</div>
